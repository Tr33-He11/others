...
int  MLHS::genSimpleBlock(){  //不需要进行isBlockValid验证
...
/********************************************/
bool MLHS::hasEnoughBox(STATE* state,BLOCK* block){     //state中是否存在足够的box
    for (int i=1;i<=nType;i++)
...
int  MLHS::generateRSBlockList(STATE* state,CUBOID rs,POINT3 rs_pos,int mxBlocks,vector<BLOCK*>* bll){
   ...
   for (int i=idx_l;i<blockTable.size();i++){
       if (!rs.contain(&(blockTable[i]->cEnvelop))) continue;  //rs太小
       if (!hasEnoughBox(state,blockTable[i]))continue;//剩余box数量不够
...
int  MLHS::placeBlock(STATE *state,BLOCK* block){ //将block放入state的栈顶。
   #ifdef DEBUG
...
int  MLHS::removeBlock(STATE *state,BLOCK* block,CUBOID rs,POINT3 rs_pos){//恢复上述操作
   for (int i=1;i<=3;i++)  state->Rs_stack.pop(),state->Rs_stack_pos.pop();   //弹出3个空间
   state->pbl.pop_back();
...
/*
int  MLHS::addBlock(STATE *state,BLOCK* block){     //将block加入state中,但不处理rs
   for (int i=1;i<=nType;i++){
       #ifdef DEBUG
       if (state->Bres[i]<block->nBoxNum[i]) exit(-2);
       #endif
...
BLOCK* MLHS::findNextBlock(STATE* s,int search_effort){ //不存在返回null
  //不存在rs，退出
...
int MLHS::accelerate(){
  ...
  for (int i=0;i<blockTable.size();i++){
    ...
    //else
    //  delete blockTable[i]; //注意pbl中保存的block可能被delete。
  }
...
void read(char *filename){
  ...
  for (int i=0;i<ks;i++){
    int t1,t2;
    scanf("%d%d",&t1,&t2);  //t1是case index，t2是随机数，无用
    data[i].read();
  }
}
MLHS mlhs;
int main(int argc,char** argv){
  ...
  if (argc<2){
      ...
      printf("如果要输出box放置策略，将main函数下data[i].output_strategy(mlhs.sol_box,mlhs.sol_boxPos)的注释取消即可");
      return 0;
  }
  read(argv[1]);
  double avgUsage=0;
  for (int i=0;i<10;i++){
    ...
    printf("[case:%d] 执行结果:%.6lf\n",i,usageRate);
    printf("         验证结果:%.6lf\n",usageRate2);       //放置方法存储在mlhs.sol_box()和mlhs.sol_bosPos()中
    //data[i].output_strategy(mlhs.sol_box,mlhs.sol_boxPos);//输出box放置策略
    avgUsage+=usageRate;
    int te=time(NULL);
    printf("         花费时间:%d s\n",te-ts);
  }
